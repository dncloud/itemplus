...
func getLabelTemplateMeta(c *gin.Context) {
	c.JSON(http.StatusOK, gin.H{
		...
		"supported_commands": printtemplates.SupportedTSPLCommands(),
		"variables":          printtemplates.LabelTemplateVariables(),
	})
}
...
func listLabelTemplates(c *gin.Context) {
	...
	if target != "" {
		if !printtemplates.IsValidLabelTemplateTarget(target) {
...
func createLabelTemplate(c *gin.Context) {
	...
	if err := printtemplates.ValidateLabelTemplateDefinitionWithDPI(name, target, *body.DPI, *body.WidthMM, *body.HeightMM, *body.GapMM, *body.Speed, *body.Density, *body.Direction, *body.CopiesDefault, tsplTemplate); err != nil {
	...
	middleware.Audit(user.ID, "label_template.create", "template_id="+strconv.Itoa(id))
...
func updateLabelTemplate(c *gin.Context) {
	...
	if err := printtemplates.ValidateLabelTemplateDefinitionWithDPI(name, target, dpi, widthMM, heightMM, gapMM, speed, density, direction, copiesDefault, tsplTemplate); err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"detail": err.Error()})
		return
	}
	...
	user := middleware.GetUser(c)
	middleware.Audit(user.ID, "label_template.update", "template_id="+strconv.Itoa(id))
...
func setDefaultLabelTemplate(c *gin.Context) {
	...
	user := middleware.GetUser(c)
	middleware.Audit(user.ID, "label_template.default", "template_id="+strconv.Itoa(id))
	respondLabelTemplate(c, id, http.StatusOK)
}
...
func deleteLabelTemplate(c *gin.Context) {
	...
	user := middleware.GetUser(c)
	middleware.Audit(user.ID, "label_template.delete", "template_id="+strconv.Itoa(id))
