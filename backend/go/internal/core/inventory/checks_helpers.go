...
func loadSessionScopeItems(realm string, locationID *int) ([]itemSnapshot, error) {
	conditions := []string{
		"NOT EXISTS (SELECT 1 FROM " + checkoutsTable(realm) + " co WHERE co.item_id = i.id AND co.status = 'active')",
	}
	args := []interface{}{}
	...
	query := fmt.Sprintf(`SELECT i.id, i.name, i.category_id,
		...
		itemsTable(realm), categoriesTable(realm), locationsTable(realm), strings.Join(conditions, " AND "))
...
func loadItemSnapshotByID(realm string, itemID int) (*itemSnapshot, error) {
	...
	query := fmt.Sprintf(`SELECT i.id, i.name, i.category_id,
		...
		LEFT JOIN %s l ON i.location_id = l.id
		WHERE i.id = ?`,
		itemsTable(realm), categoriesTable(realm), locationsTable(realm))
	...
	res, err := database.DB.Exec(
		`INSERT INTO `+sessionEntryTable()+`
			(session_id, item_id, item_name, category_id, category_name, category_color,
			 location_id, location_name, location_color, expected_in_scope, status, found_via,
