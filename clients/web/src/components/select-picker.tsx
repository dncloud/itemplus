...
export default function SelectPicker({ label, value, onChange, options, placeholder = "—", clearLabel = "—", searchable = true }: SelectPickerProps) {
  ...
  return (
    <div className={clsx("relative", open && "z-[60]")} ref={containerRef}>
      {label && <label className="mb-2 inline-block text-sm/6 font-medium text-gray-900 dark:text-white">{label}</label>}
      <button
        type="button"
        onClick={() => { setOpen(!open); setSearch(""); setTimeout(() => inputRef.current?.focus(), 50); }}
        className="group flex w-full items-center justify-between gap-2 rounded-lg border border-gray-200 bg-white px-3 py-2 text-left leading-6 focus:border-gray-500 focus:ring-3 focus:ring-gray-500/50 focus:outline-hidden dark:border-gray-600 dark:bg-gray-800 dark:focus:border-gray-500"
      >
        <span className={`truncate text-sm ${selectedName ? "text-gray-900 dark:text-gray-100" : "text-gray-500 dark:text-gray-400"}`}>
      ...
      {open && (
        <ul
          className="absolute inset-x-0 z-[70] mt-2 max-h-60 origin-top overflow-y-auto rounded-lg bg-white py-2.5 shadow-xl ring-1 ring-black/5 focus:outline-hidden dark:bg-gray-800 dark:shadow-gray-900 dark:ring-gray-700"
          role="listbox"
          tabIndex={0}
        >
          ...
          >
            ...
            <div className={clsx("pointer-events-none size-5 flex-none text-gray-600 dark:text-gray-500", value == null ? "visible" : "invisible")} aria-hidden="true">
              <svg
          ...
          {filtered.map((opt) => {
            ...
            return (
              ...
              >
                <div className="grow truncate py-1.5">{opt.name}</div>
                <div
                  className={clsx("pointer-events-none size-5 flex-none text-gray-600 dark:text-gray-500", active ? "visible" : "invisible")}
                  aria-hidden="true"
                >
                  <svg
                    className="inline-block h-5 w-5"
