...
import { HLSPlayer } from "@/components/attachments/hls-player";
...
} from "@/components/attachments/attachment-manager-preview";
    ...
    <div ref={setNodeRef} style={style} className="overflow-hidden rounded-xl border border-gray-200 bg-white transition-colors hover:bg-gray-50 dark:border-white/10 dark:bg-gray-800/50 dark:hover:bg-white/2.5">
      <div className="flex items-center gap-2 px-4 py-4">
        ...
        {isPreviewable ? (
          ...
          >
            {showPlayer ? <X className="h-4 w-4" /> : isImgFile ? <ImageIcon className="h-4 w-4" /> : isAudio ? <Music className="h-4 w-4" /> : (isVideo || isHLS) ? <Video className="h-4 w-4" /> : <FileText className="h-4 w-4" />}
          </button>
        ) : null}
        {!readOnly ? (
          <>
