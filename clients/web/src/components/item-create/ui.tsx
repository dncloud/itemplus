...
export function TWPSelect({
  ...
  options: Array<{ id: number; name: string; logo?: string; logoBackground?: string }>;
  disabled?: boolean;
}) {
  ...
  const renderOptionVisual = (option?: { logo?: string; logoBackground?: string }, active?: boolean) => {
    if (option?.logo) {
      return (
        <span
          className="flex size-5 items-center justify-center overflow-hidden rounded-md border border-gray-200 dark:border-gray-700"
          style={option.logoBackground ? { backgroundColor: option.logoBackground } : undefined}
        >
          <img src={option.logo} alt="" className="h-full w-full object-contain p-0.5" />
        </span>
      );
    }
    return (
      ...
      <div className="mt-2 block">
        ...
        >
          <span className="col-start-1 row-start-1 flex min-w-0 items-center gap-2 pr-6">
            {renderOptionVisual(selected, Boolean(selected))}
              ...
              return (
                ...
                >
                  <span className="flex min-w-0 items-center gap-2">
                    ...
                    <span className={clsx("block whitespace-nowrap pr-6 font-normal", selectedOption && "font-semibold")}>
                      {option.name}
                    </span>
                  </span>
                </button>
...
export function TWPStringSelect({
  ...
  return (
    <div ref={wrapperRef} className="relative">
      {label ? <label className="block text-sm/6 font-medium text-gray-900 dark:text-white">{label}</label> : null}
      <div className="mt-2 block">
        <button
          type="button"
          aria-haspopup="listbox"
