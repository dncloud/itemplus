...
import { ChoiceTile, SelectField, SettingsCard, StatusMessage, ToggleRow } from "@/components/settings-ui";

type PromptTab = "chat" | "parse" | "category" | "property";
...
export function SettingsAISection({
  ...
  const selectedModelOption = selectedProfile && loadedModels.some((option) => option.id === selectedProfile.model) ? selectedProfile.model : "";
  ...
  const onLoadModels = async () => {
    ...
    try {
      ...
      // handled in the parent status bar already
    }
  };
              ...
              return (
                ...
                >
                  <div className="flex items-center justify-between gap-2">
                    <div className="min-w-0">
                      <div className="truncate text-sm font-medium">{profile.name || t("settings.aiUnnamedProfile")}</div>
          ...
          <div className="space-y-8">
                    ...
                    {selectedProfile.provider === "openai" && loadedModels.length > 0 ? (
                      <div className="mb-3">
                        <SelectField
                          ...
                          onChange={(value) =>
                            updateSelectedProfile((profile) => ({ ...profile, model: String(value) }))
                          }
              ...
              <div className="space-y-4 border-t border-gray-200 pt-6 dark:border-gray-700">
                <div>
                  <p className="text-sm/6 font-medium text-gray-900 dark:text-white">
                    {promptTabs.find((tab) => tab.id === activePromptTab)?.label}
                  </p>
                  <p className="mt-1 text-sm/6 text-gray-500 dark:text-gray-400">
                    {promptTabs.find((tab) => tab.id === activePromptTab)?.hint}
                  </p>
                </div>
                <textarea
                  ...
                  onChange={(e) =>
                    updateSelectedProfile((profile) => {
                      ...
                      return { ...profile, property_enhancement_prompt: e.target.value };
                    })
                  }
                  rows={14}
                  ...
                  <button
                    ...
                    onClick={() =>
                      updateSelectedProfile((profile) => {
                        ...
                        return { ...profile, property_enhancement_prompt: nextValue };
                      })
                    }
                    className={secondaryButtonClass}
                  >
            ...
            <div className="flex flex-wrap items-center gap-2">
              ...
              {aiStatus ? (
                <StatusMessage
                  tone={
                    aiStatus.startsWith(t("settings.aiTestSucceeded")) || aiStatus === t("settings.aiSaved")
                      ? "success"
                      : "error"
                  }
                >
